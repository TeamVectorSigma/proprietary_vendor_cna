...
PRODUCT_PROPERTY_OVERRIDES += \
ro.cnaversion=CNA-JELLY-($(BRANCH))-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(shell date +%m%d%Y).$(PRODUCT_VERSION_DEVICE_SPECIFIC)-Test

ifdef CNA_NIGHTLY
    PRODUCT_PROPERTY_OVERRIDES += \
            ro.cnaversion=CNA-JELLY($(BRANCH))-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(shell date +%m%d%Y)-$(PRODUCT_RELEASE_NAME)-NIGHTLY
else
    ifdef CNA_RELEASE
        PRODUCT_PROPERTY_OVERRIDES += \
